...
def main():
    ...
    if config.GITHUB_PR_NUMBER:
        ...
        print("trying to read previous coverage rate")
        previous_coverage_rate = get_previous_coverage_rate_from_xml(config=config)
        print(f"previous coverage rate: {previous_coverage_rate}")
...
class Config:
    ...
    GITHUB_REF: str
    GITHUB_TMP_BRANCH: str = "tempmaster"
...
def get_diff_coverage_info(config: Config) -> dict:
    ...
    print("printing diff-cover results:\n--------------------------\n")
    os.system('diff-cover cobertura.xml --compare-branch tempmaster')
    xml = ET.parse(config.COVERAGE_FILE)
    root = xml.getroot()
    xml_source_path = root.find('./sources/source').text
    print(f"current branch coberture.xml source path: {xml_source_path}")
...
def get_previous_coverage_rate(config: Config) -> Optional[float]:
    try:
        ...
        print(f"Error - get_previous_coverage_rate exception: {e}", e.with_traceback())
        print("Previous coverage results not found, cannot report on evolution.")
        return None


def get_previous_coverage_rate_from_xml(config: Config) -> Optional[float]:
    try:
        print("starting to retrieve previous coverage rate from xml")
        xml_string = subprocess.check_output(
            f"git show {config.GITHUB_TMP_BRANCH}:{config.COVERAGE_FILE}", shell=True
        )
        # print cobertura.xml path
        xml_root = ET.ElementTree(ET.fromstring(xml_string.decode("UTF-8"))).getroot()
        xml_source_path = xml_root.find('./sources/source').text
        print(f"previous branch coberture.xml source path: {xml_source_path}")
        line_rate = xml_root.get("line-rate")
        return float(line_rate)
    except Exception as e:
        print(f"Error - get_previous_coverage_rate_from_xml exception: {e}")
